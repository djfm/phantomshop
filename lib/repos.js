...
module.exports.Repo = Repo = function (directory, branch, remote, remoteName, remoteBranch, distance)
{
	...
	this.getSubmodulesBranch = function ()
	{
		var p = path.join(directory, '.submodules_branch');
...
var buildRepo;
module.exports.buildRepo = buildRepo = function (directory, options)
{
	var d = new Deferred();
	...
	git.isRepository(directory).then(function (yes) {
		if (yes)
		{
			(options.fetch? git.fetch(directory) : tools.makeEmptyPromise()).then(function () {
